...
class AIService {
  ...
  // Several screens call static methods on AIService (legacy names).
  // Provide simple wrappers mapping to the instance methods above.

  static Future<String> generateGuidedQuestion(List<String> messages, String stage) async {
    // Keep signature async to match callers; return a guiding question.
    final service = AIService();
    return service.getGuidingQuestion();
  }

  static List<String> generateBondingActivities({int count = 3}) {
    final service = AIService();
    return service.getBondingActivities(count: count);
  }
  ...
  static Map<String, double> calculateCommunicationScores(
    ...
    // Lightweight synchronous scoring returning a simple map of numeric metrics.
    final service = AIService();
    final random = Random();

    final partnerScore = service._generateVoiceBasedScore(true, random, true);

    return {
      'empathy': partnerScore.empathy,
      'listening': partnerScore.listening,
      'reception': partnerScore.reception,
      'clarity': partnerScore.clarity,
      'respect': partnerScore.respect,
      'responsiveness': partnerScore.responsiveness,
      'openMindedness': partnerScore.openMindedness,
      'average': partnerScore.averageScore,
    };
  }
  ...
  static Future<String> generateSessionSummary(
    ...
    final avgA = partnerAScores['average'] ?? 0.0;
    final avgB = partnerBScores['average'] ?? 0.0;
    final overall = ((avgA + avgB) / 2 * 100).toStringAsFixed(0);

    final summaryBuffer = StringBuffer();
    summaryBuffer.writeln('Session Summary — Duration: ${durationSeconds}s');
    summaryBuffer.writeln('Overall communication score: $overall%');
    summaryBuffer.writeln(service.getCelebratoryMessage());
    summaryBuffer.writeln('Suggested improvements: ${service._generateImprovementSuggestions().join(", ")}');
    summaryBuffer.writeln('\nSuggested bonding activities: ${service.getBondingActivities(count: 3).join(" • ")}');
